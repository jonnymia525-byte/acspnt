...
export async function POST(req: Request) {
  try {
    ...
    if (action === "check_duplicates") {
      const { productId, accounts: accountsData } = body;
      ...
      for (let i = 0; i < newAccounts.length; i++) {
        ...
        if (existingLines.has(normalized)) {
          ...
          duplicates.push({ line: newAccounts[i], index: i, productId: foundIn?.id, productTitle: foundIn?.title });
        }
      }
    ...
    // ─── UPLOAD ACCOUNTS (default action) ──────────────────────────
    const { productId, accountsData } = body;
